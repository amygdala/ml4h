...
from ml4h.plots import evaluate_predictions, plot_scatters, plot_rocs, plot_precision_recalls, subplot_roc_per_class, plot_tsne, plot_prediction_calibrations, \
    plot_reconstruction, plot_hit_to_miss_transforms, plot_survival
        ...
        while True:
            ...
            for ot in model.output_names:
                ...
                if len(otm.shape) == 1 and otm.is_continuous():
                    ...
                    logging.info(f"predicted_survivals on {1-predicted_survivals[0, -1]} sick[0]: {sick[0]}")
...
def saliency_maps(args):
    ...
    model = make_multimodal_multitask_model(**args.__dict__)
    data, labels, paths = big_batch_from_minibatch_generator(generate_test, args.test_steps)
    input_map = args.tensor_maps_in[0]
    in_tensor = data[input_map.input_name()]
    for tm in args.tensor_maps_out:
        if len(tm.shape) > 1:
            continue
...
def _predict_and_evaluate(
    ...
    for y, tm in zip(y_predictions, tensor_maps_out):
        ...
        if tm.is_language():
            sample_from_language_model(tensor_maps_in[0], model, test_data, max_samples=136)
