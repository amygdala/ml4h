...
def plot_metric_history(history, training_steps: int, title: str, prefix='./figures/'):
    row = 0
    col = 0
    total_plots = math.ceil(int(len(history.history) / 2))  # divide by 2 because we plot validation and train histories together
...
def plot_survival(
    ...
    plt.title(f'{title}\nEnrolled: {truth.shape[0]}, Censored: {cumulative_censored[-1]:.0f}, {100 * (cumulative_censored[-1] / truth.shape[0]):2.1f}%, '
              ...
              f'Max follow up: {days_window} days, {days_window // 365} years. C-Index: {c_index}')
    logging.info(f'Enrolled: {truth.shape[0]}, Censored: {cumulative_censored[-1]:.0f}, {100 * (cumulative_censored[-1] / truth.shape[0]):2.1f}%')
    logging.info(f'Events: {cumulative_sick[-1]:.0f}, {100 * (cumulative_sick[-1] / truth.shape[0]):2.1f}%')
        ...
        if i % sample_every == 0 and col < frames:
            for j in range(rows):
                if len(test_data[tm.input_name()].shape) == 4:
                    ...
                    for l in range(12):
                        axes[j, col].plot(range(2400), (5*l)+test_data[tm.input_name()][j, :2400, l])
                        axes[j, col].set_yticks(())
            col += 1
...
def plot_reconstruction(
    ...
    for i in range(min(len(paths), num_samples)):
        ...
        if tm.axes() == 2:
            ...
            fig, axes = plt.subplots(tm.shape[1], 3, figsize=(3 * SUBPLOT_SIZE, 6*SUBPLOT_SIZE)) #, sharey=True)
        ...
        for i in range(samples):
            if dtm.axes() == 3:
                ...
                if dtm.is_categorical():
                    axes[i, 0].imshow(np.argmax(input_data[dtm.input_name()][i, ...], axis=-1), cmap=cmap)
                    axes[i, 1].imshow(np.argmax(predictions[i, ...], axis=-1), cmap=cmap)
                    if sexes[i] >= thresh:
                        axes[i, 2].imshow(np.argmax(f2m[i, ...], axis=-1), cmap=cmap)
                    ...
                    if sexes[i] >= thresh:
                        axes[i, 2].imshow(f2m[i, ..., 0], cmap='gray')
                        axes[i, 2].set_title(f'Transform {feature[:16]} to less than {thresh:.2f}')
                    else:
